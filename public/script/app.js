...
function Game() {
	...
	this.start = function(roomIndicator, userIndicator){
		//users = newUsers ? newUsers : [];
		//typeUser = [0, 1, 2] Local/ Network/ Bot

		//TODO: Firebase players
		let dataString = 'Rooms/' + roomIndicator + '/Game/Players/';
		for (let i = 0; i < userIndicator.length; i++) {
			database.ref(dataString + userIndicator[i] + '/Action').set('start');
			{
				let testUserAgent = new LocalAgent();
				let userNetworkAgent = new NetworkAgent();
				let testUser = new User(users.length === 0);
	...
	function lose() {
		...
		//TODO: Firebase active game = false
	}
	this.lose = lose;
		...
		function select(userId = 0) {
			...
			users[userId].selectCards( users.length === 1 ? (isFirstRound ? 2 : 4) : 1, selectionCards, function(selectedCards) {
				...
				if (isFirstRound && countGived < (users.length * 2) || !isFirstRound && countGived < 4) {
					...
					//TODO: Firebase user done action = chooseCards --> wait all users
		...
		for (let user of users) {
			user.programming(function() {
				...
				if (countUsers === users.length) {
					//TODO: Firebase user done action = programming --> wait all users
	...
	function warriorsAct() {
		...
		async function go(userId = 0) {
			...
			for (let stack of users[userId].stacks) {
				...
				//TODO: Firebase user-X done action = warriorAct
			}
	...
	function runStack(user, stack){
		...
		//TODO: add action warriorAct --> wait all users
	}
	...
	function creepsMoveAct() { // TODO: таранят бомбу или героя если они на пути
		...
		//TODO: add action creepsMoveAct --> wait all users
	...
	function creepsSpawnAct() {
		...
		//TODO: add action creepsSpawnAct --> wait all users
	...
	function creepsAttackAct() {
		...
		function attack(eventId = 0) {
			...
			if (eventId + 1 < attackEvent.length) {
				...
				//TODO: add action creepsAttackAct --> wait all users
				finalAct();
			}
